...
mod tree_sitter_bindings {
    ...
    extern "C" {
        /// Returns the Tree-sitter Language for MML grammar.
        ///
        /// # Safety
        /// This is safe because the tree-sitter library manages the `Language`
        /// struct internally, the returned `Language` has static lifetime, and
        /// the `tree-sitter-mml` parser has been correctly generated by the
        /// tree-sitter CLI and linked into this crate via `build.rs`.
...
fn parse_node(node: &Node, source: &str) -> Result<Option<AstToken>, String> {
    match node.kind() {
        ...
        "track_separator" => {
            let length = node.end_byte() - node.start_byte();
            Ok(Some(AstToken::TrackSeparator(TrackSeparatorToken {
                length,
            })))
        }
