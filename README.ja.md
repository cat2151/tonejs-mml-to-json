...
# 状況
- このドキュメントはまだAI生成を含むため、今後の実装予定まで間違っている可能性があります。
- 今後修正予定です。
...
| コマンド | 説明 | 例 |
|---------|------|-----|
| `'音符'` | 和音（シングルクォートで囲まれた音符が同時に演奏される）<br>臨時記号、長さ、付点を指定可能 | `'ceg'` `'c+eg-'4` `'dfac'8.` |
...
- **機能**:
  ...
  - 長さと付点のサポート: `'ceg'4.` = 付点4分音符のC-E-G和音
...
- **将来**: 名前指定（例: `@synth`, `@fm`, `@am`）または、MML内で予め定義した音色を数値で呼び出す方式を検討中
...
|---------------|----------------------|------------------|------|
| **Synth** | ✅ 対応済み | ✅ 対応済み | `@` コマンドで基本実装済み |
| **MonoSynth** | ✅ 対応済み | ⏳ 未対応 | ベース音色に適している |
| **FMSynth** | ✅ 対応済み | ⏳ 未対応 | FM音色（エレピ、ベルなど） |
| **AMSynth** | ✅ 対応済み | ⏳ 未対応 | AM音色（ベル、金属音など） |
| **DuoSynth** | ✅ 対応済み | ⏳ 未対応 | デュアルボイス、豊かな音色 |
| **PluckSynth** | ✅ 対応済み | ⏳ 未対応 | ギター、ハープなどの撥弦楽器 |
| **MembraneSynth** | ✅ 対応済み | ⏳ 未対応 | ドラム、パーカッション |
| **MetalSynth** | ✅ 対応済み | ⏳ 未対応 | シンバル、金属打楽器 |
| **NoiseSynth** | ✅ 対応済み | ⏳ 未対応 | ノイズベース音色 |
| **PolySynth** | ✅ 対応済み | ✅ 対応済み | 和音機能で実装済み |
...
1. **音色（Instrument）の拡張**
   ...
   - 名前指定: `@synth`, `@fm`, `@am`, `@pluck`, `@membrane`, `@metal` など
   - 数値指定: MML内で予め定義した音色を番号で呼び出す方式（ユーザー定義可能）
