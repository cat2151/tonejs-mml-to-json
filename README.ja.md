...
| `+` `-` | 臨時記号（シャープ/フラット）<br>音符の直後に記述 | `c+` `e-` `c++` `e--` |
...
| `PluckSynth` | カルプス・ストロング法<br>撥弦楽器シミュレーション | ギター、ハープ、琴、撥弦系 |
...
- Tone.jsのデフォルト音色表現バリエーションを一通り簡易的に鳴らして検証するための仮仕様です
